            ...
            parser: Rc::new(move |input: Input| match parser1.parse(input.clone()) {
                ...
                Err(_) => {
                    match parser2.parse(input) {
                        Ok((right_result,remaining)) => {Ok((EitherType::Right(right_result),remaining))},
                        Err(_) => {Err("error")},
                    }
...
impl<'a, Input: Debug + Clone + 'a, T1, T2> Parse<'a, Input, EitherType<T1,T2>> for Either<'a, Input, T1, T2>
where
    T1: Debug + Clone,
    T2: Debug + Clone,
{
